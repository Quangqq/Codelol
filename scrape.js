...
const outputFile = 'tipe1-proxy.txt';
...
const downloadAndSaveProxies = async (url, outputFile) => {
    try {
        ...
        if (response.status === 200) {
            ...
            fs.appendFileSync(outputFile, proxies.join('\n') + '\n');
            console.log(`Success: ${url} - ${proxies.length} proxies`);
...
(async () => {
    ...
    for (let url of proxyUrls) {
        const proxyCount = await downloadAndSaveProxies(url, outputFile);
        totalProxies += proxyCount;
    }

    const fileContent = fs.readFileSync(outputFile, 'utf8');
    const lines = fileContent.split('\n');
    const uniqueProxies = [...new Set(lines)].filter((line) => line.trim() !== '');
    fs.writeFileSync(outputFile, uniqueProxies.join('\n'));
